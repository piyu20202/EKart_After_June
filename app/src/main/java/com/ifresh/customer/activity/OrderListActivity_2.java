    ...
    private void makejsonArr(String res)
    {
        ...
        try{
            ...
            for(int i = 0; i<data_arr.length(); i++)
            {
                ...
                if (jsonobj.has("review") && !jsonobj.isNull("review"))
                {
                    ...
                    jsonobj.put("product_rate", data_arr.getJSONObject(i).getJSONObject("review").getString("product_rate"));
                    jsonobj.put("dboy_rate", data_arr.getJSONObject(i).getJSONObject("review").getString("dboy_rate"));
                    jsonobj.put("comment", data_arr.getJSONObject(i).getJSONObject("review").getString("comment"));
                    jsonobj.put("why_low_rate", data_arr.getJSONObject(i).getJSONObject("review").getString("why_low_rate"));

                }
                else
                {
                    obj_reivew.put("product_rate", "0");
                    obj_reivew.put("dboy_rate", "0");
                    obj_reivew.put("comment", "");
                    obj_reivew.put("why_low_rate", "");
                    jsonobj.put("product_rate", "0");
                    jsonobj.put("dboy_rate", "0");
                    jsonobj.put("comment", "");
                    jsonobj.put("why_low_rate", "");
                }

                array_review.put(0, obj_reivew);
                ...
                /*jsonobj.put("revised_price", data_arr.getJSONObject(i).getJSONObject("order_variants")..getString("revised_price"));
                jsonobj.put("revised_qty", data_arr.getJSONObject(i).getJSONObject("order_variants").getString("revised_qty"));*/


                jsonobj.put("final_total", data_arr.getJSONObject(i).getString("final_total"));

                if(data_arr.getJSONObject(i).has("delivery_total"))
                {
                    jsonobj.put("delivery_total", data_arr.getJSONObject(i).getString("delivery_total"));
                }
                else{
                    jsonobj.put("delivery_total", "0");
                }

                if(data_arr.getJSONObject(i).has("received_total"))
                {
                    jsonobj.put("received_total", data_arr.getJSONObject(i).getString("received_total"));
                }
                else{
                    jsonobj.put("received_total", "0");
                }
                ...
                for(int j = 0; j<order_variants_arr.length(); j++)
                {
                    ...
                    if(order_variants_arr.getJSONObject(j).has("revised_unit"))
                    {
                        if(order_variants_arr.getJSONObject(j).getString("revised_unit").equalsIgnoreCase(""))
                        {
                            mjson_obj_item.put("revised_unit", "");
                        }
                        else{
                            mjson_obj_item.put("revised_unit", order_variants_arr.getJSONObject(j).getString("revised_unit"));
                        }
                    }
                    else{
                        mjson_obj_item.put("revised_unit", "");
                    ...
                    if(order_variants_arr.getJSONObject(j).has("revised_measurement"))
                    {
                        if(order_variants_arr.getJSONObject(j).getString("revised_measurement").equalsIgnoreCase(""))
                        {
                            mjson_obj_item.put("revised_measurement", "");
                        }
                        else{
                            mjson_obj_item.put("revised_measurement", order_variants_arr.getJSONObject(j).getString("revised_measurement"));
                        }
                    }
                    else{
                        mjson_obj_item.put("revised_measurement", "");
                    ...
                    /*if(order_variants_arr.getJSONObject(j).has("revised_measurement"))
                    {
                        mjson_obj_item.put("revised_unit", order_variants_arr.getJSONObject(j).getString("revised_measurement"));
                    }
                    else{
                        mjson_obj_item.put("revised_unit", "0");
                    }
                    ...
                    if(order_variants_arr.getJSONObject(j).has("revised_unit"))
                    {
                        ...
                        if(order_variants_arr.getJSONObject(j).getString("revised_unit").equalsIgnoreCase("1"))
                        {
                            value = "kg(s).";
                            break;
                        }
                        else if(order_variants_arr.getJSONObject(j).getString("revised_unit").equalsIgnoreCase("2"))
                        {
                            value = "gm";
                            break;
                        }
                        else if(order_variants_arr.getJSONObject(j).getString("revised_unit").equalsIgnoreCase("3"))
                        {
                            value = "ltr";
                            break;
                        }
                        else if(order_variants_arr.getJSONObject(j).getString("revised_unit").equalsIgnoreCase("4"))
                        {
                            value = "ml";
                            break;
                        }
                        else if(order_variants_arr.getJSONObject(j).getString("revised_unit").equalsIgnoreCase("5"))
                        {
                            value = "pack";
                            break;
                        }
                        else if(order_variants_arr.getJSONObject(j).getString("revised_unit").equalsIgnoreCase("6"))
                        {
                            value = "pcs";
                            break;
                        }
                        else if(order_variants_arr.getJSONObject(j).getString("revised_unit").equalsIgnoreCase("7"))
                        {
                            value = "m";
                            break;
                        }
                        mjson_obj_item.put("revised_measurement", value);


                    }
                    else{
                        mjson_obj_item.put("revised_measurement", "0");
                    }
*/



                    if(order_variants_arr.getJSONObject(j).has("revised_qty"))
                    {
                        if(order_variants_arr.getJSONObject(j).getString("revised_qty").equalsIgnoreCase(""))
                        {
                            mjson_obj_item.put("revised_qty", "");
                        }
                        else{
                            mjson_obj_item.put("revised_qty", order_variants_arr.getJSONObject(j).getString("revised_qty"));
                        }
                    }
                    else{
                        mjson_obj_item.put("revised_qty", "");
                    ...
                    if(order_variants_arr.getJSONObject(j).has("revised_price"))
                    {
                        if(order_variants_arr.getJSONObject(j).getString("revised_price").equalsIgnoreCase(""))
                        {
                            mjson_obj_item.put("revised_price", "");
                        }
                        else{
                            mjson_obj_item.put("revised_price", order_variants_arr.getJSONObject(j).getString("revised_price"));
                        }
                    }
                    else{
                        mjson_obj_item.put("revised_price", "");
                    ...
                    if(order_variants_arr.getJSONObject(j).has("revised_status"))
                    {
                        if(order_variants_arr.getJSONObject(j).getString("revised_status").equalsIgnoreCase(""))
                        {
                            mjson_obj_item.put("revised_status", "");
                        }
                        else{
                            mjson_obj_item.put("revised_status", order_variants_arr.getJSONObject(j).getString("revised_status"));
                        }
                    }
                    else{
                        mjson_obj_item.put("revised_status", "");
            ...
            for (int i = 0; i < jsonArray.length(); i++)
            {
                ...
                for (int j = 0; j < itemsarray.length(); j++) {
                    ...
                    itemList.add(new OrderTracker_2(
                            ...
                            itemobj.getString("revised_unit"),
                            itemobj.getString("revised_measurement"),
                            itemobj.getString("revised_qty"),
                            itemobj.getString("revised_price"),
                            itemobj.getString("revised_status"),
                ...
                OrderTracker_2 orderTracker = new OrderTracker_2(
                        ...
                        jsonObject.getString("product_rate"),
                        jsonObject.getString("dboy_rate"),
                        jsonObject.getString("comment"),
                        jsonObject.getString("why_low_rate"),
                        ...
                        jsonObject.getString("delivery_total"),
                        jsonObject.getString("received_total"),
    ...
    private void Call_ordertracker_api() {
        ...
        Log.d("user_track",session.getData(Session.KEY_id));
