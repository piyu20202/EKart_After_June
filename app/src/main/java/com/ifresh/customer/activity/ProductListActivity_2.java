...
public class ProductListActivity_2 extends AppCompatActivity {
    ...
    int  endingPoint = 0, stepCount=20, totalCount =0;
    String where="";

    String pass_category_id="";
    int pass_endingPoint;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ApiConfig apiConfig = new ApiConfig(mContext);
        ...
        recycler_View_ver.addOnScrollListener(new RecyclerView.OnScrollListener()
        {
            ...
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                ...
                if (!recyclerView.canScrollVertically(1)&& newState==RecyclerView.SCROLL_STATE_IDLE) {
                    ...
                    if (endingPoint <= totalCount)
                    {
                        endingPoint = endingPoint+stepCount;
                        callApiProductlist(pass_category_id,false, false,endingPoint);
                    }
    ...
    private void callSettingApi_messurment()
    {
        ...
        {
            endingPoint = 0;
            callApiProductlist(category_id,true, true,endingPoint);
    ...
    public void callApiProductlist(final String category_id, final boolean is_callsubcat, boolean is_productlist_clear, int endingPoint)
    {
        Log.d("cat_id&&&&",category_id);
        pass_category_id = category_id;
        pass_endingPoint = endingPoint;

        String ProductListUrl = BASEPATH + GET_PRODUCTLIST + session.getData(Constant.AREA_ID) +"/"+ category_id+"?start="+endingPoint+"&where="+"" ;
        Log.d("url", ProductListUrl);
        //Log.d("endingPoint", ""+endingPoint);
                        ...
                        if (object.getInt(Constant.SUCESS) == 200)
                        {
                            ...
                            if(is_callsubcat)
                            {
                               ...
                               if(is_productlist_clear)
                               {
                                   ...
                                   {
                                       //No data in product list show no data
                                       progressBar.setVisibility(View.GONE);
                               ...
                               // Log.d("name====>", arrayList_product.get(arrayList_product.size()-1).getName());
                                ...
                                /*if(arrayList_product.size() > 0)
                                {
                                    ...
                                    if(is_productlist_clear)
                                    {
                                        ...
                                        // data exist already append data
                                    ...
                                    Toast.makeText(mContext, "no data for product list", Toast.LENGTH_SHORT).show();
                                    nodata_view.setVisibility(View.VISIBLE);
                                    recycler_View_ver.setVisibility(View.GONE);
                                  }*/
                ...
                try{
                    if( arrayList_product != null && arrayList_product.size() > 0 )
                    {
                        startActivity(new Intent(ProductListActivity_2.this, SearchActivity_2.class)
                                ...
                                .putExtra("url",BASEPATH + GET_PRODUCTLIST + session.getData(Constant.AREA_ID) +"/"+ pass_category_id+"?start="+pass_endingPoint+"&where=")
                        );
