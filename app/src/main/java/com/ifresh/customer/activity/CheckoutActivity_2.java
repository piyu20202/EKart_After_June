...
public class CheckoutActivity_2 extends AppCompatActivity implements OnMapReadyCallback, PaymentResultListener
{
    ...
    LinearLayout lytPayOption, lytTax, lytOrderList, lytWallet, lytCLocation, paymentLyt, deliveryLyt, lytPayU, lytPayPal, lytPayTm, lytRazorPay, dayLyt,linear_adtype,linear_view;
    Button btnApply;
    ...
    String payTmpaymentResponse = "",is_wholesaler = "0";
    ...
    String paymentMethod_id,checkSum,token;
    RadioButton rbCod, rbPayU, rbPayPal, rbRazorPay, rbPayTm;
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        lytPayTm = findViewById(R.id.lytPayTm);
    ...
    private void GetPayment_methodtype()
    {
        try{
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                payment_typeList.add(new PaymentType(object1.getString("id"), object1.getString("title"), object1.getString("abv"),object1.getBoolean("flag")));
            }
        }
    ...
    public void setPaymentMethod()
    {
        ...
        for (int i= 0; i<payment_typeList.size(); i++)
        {
            if (payment_typeList.get(i).getTitle().equalsIgnoreCase("RazorPay"))
            {
                if (payment_typeList.get(i).getFlag())
                {
            ...
            if (payment_typeList.get(i).getTitle().equalsIgnoreCase("Paytm"))
            {
                if (payment_typeList.get(i).getFlag())
                {
                ...
                {
                    lytPayTm.setVisibility(View.GONE);
                }
            }
        }
        ...
        rbPayTm.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    public void PlaceOrderProcess() {
        ...
        try{
            ...
            obj_sendParam.put("is_wholesaler",is_wholesaler);
    ...
    public void CreatePayTmOrderId()
    {
        /*if (Constant.APP_URL.equalsIgnoreCase("staging")) {
            ...
            Constant.PAYTM_WEBSITE_NAME = "APPSTAGGING";
        } else {

            host = "https://securegw.paytm.in/";
        }*/
        final String orderId = String.valueOf(System.currentTimeMillis());
        Call<ChecksumResponse> call = ApiClient.getClient().getCheckSum(orderId,Constant.merchant_id,session.getData("_id"),"WAP","Retail",Constant.PAYTM_WEBSITE_NAME, String.valueOf(subtotal),varifyUrl);
        call.enqueue(new Callback<ChecksumResponse>() {
            ...
            public void onResponse(Call<ChecksumResponse> call, Response<ChecksumResponse> response) {
                if (response.isSuccessful())
                {
                    if (response.body().getChecksum()!= null)
                    {
                        ...
                        launchPayTMCheckout(subtotal.toString(),checkSum,token,orderId);
                    }
                    else
                    {
                        Toast.makeText(CheckoutActivity_2.this, ""+response.body().toString(), Toast.LENGTH_SHORT).show();
                    }
                }
                else
                {
                    Toast.makeText(CheckoutActivity_2.this, ""+response.message(), Toast.LENGTH_SHORT).show();
                }
            ...
            public void onFailure(Call<ChecksumResponse> call, Throwable t) {
                Toast.makeText(CheckoutActivity_2.this, ""+t, Toast.LENGTH_SHORT).show();
            }
        });
    }


    private void launchPayTMCheckout(String totalPrice,String checkSum,String token,String orderid) {

        // Create map of input parameters
        HashMap<String, String> paramMap = new HashMap<String, String>();
        paramMap.put("MID", Constant.merchant_id);
        paramMap.put("ORDER_ID", orderid);
        paramMap.put("CUST_ID", session.getData("_id"));
        paramMap.put("TXN_AMOUNT", totalPrice);
        paramMap.put("Currency", "INR");
        paramMap.put("CHANNEL_ID","WAP");
        paramMap.put("WEBSITE", Constant.PAYTM_WEBSITE_NAME);
        ...
        host = Constant.PAYTM_HOST;

        String callBackUrl = varifyUrl+orderid;
        PaytmOrder paytmOrder = new PaytmOrder(orderid,Constant.merchant_id, token, totalPrice, callBackUrl);
        TransactionManager transactionManager = new TransactionManager(paytmOrder, new PaytmPaymentTransactionCallback() {
            @Override
            public void onTransactionResponse(@Nullable Bundle bundle) {

                Map<String, String> payTm = new HashMap<>();
                ...
                if (bundle.getString("STATUS").equalsIgnoreCase("TXN_SUCCESS"))
                {
                    ...
                    JSONObject obj_sendParamPay = new JSONObject();
                    try{
                        obj_sendParamPay.put("ORDERID",orderId);
                        obj_sendParamPay.put("MID",mid);
                        obj_sendParamPay.put("TXNID",txnId);
                        obj_sendParamPay.put("TXNAMOUNT",txnAmount);
                        obj_sendParamPay.put("PAYMENTMODE",paymentMode);
                        obj_sendParamPay.put("CURRENCY",currency);
                        obj_sendParamPay.put("TXNDATE",txnDate);
                        obj_sendParamPay.put("STATUS",status);
                        obj_sendParamPay.put("RESPCODE",response);
                        obj_sendParamPay.put("RESPMSG",responseMsg);
                        obj_sendParamPay.put("GATEWAYNAME",wallet);
                        obj_sendParamPay.put("BANKTXNID",bankTxtId);
                        obj_sendParamPay.put("BANKNAME",bankName);
                        obj_sendParamPay.put("CHECKSUMHASH",checkSum);
                        obj_sendParamPay.put("deviceType","ANDROID");

                        obj_sendParam.put("paytm_status", obj_sendParamPay);
                        obj_sendParam.put("paytm_payment_id", txnId);
                        payTm.put("order_param", obj_sendParam.toString());
                    }
                    catch (Exception ex)
                    {
                        ex.printStackTrace();
                        Toast.makeText(CheckoutActivity_2.this, ""+ex, Toast.LENGTH_LONG).show();
                    }
                    ...
                    }, CheckoutActivity_2.this, Constant.BASEPATH + Constant.GET_ORDERSEND,  payTm, false);

                }
                else
                {
                    Toast.makeText(CheckoutActivity_2.this, "Payment not done...", Toast.LENGTH_SHORT).show();
                }
            ...
            public void onErrorProceed(String s) {
                Toast.makeText(CheckoutActivity_2.this, ""+s, Toast.LENGTH_SHORT).show();
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == ActivityRequestCode && data != null) {
            ...
            payTmpaymentResponse = data.getStringExtra("response");
